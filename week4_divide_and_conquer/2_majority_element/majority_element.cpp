...
int get_majority_element(vector<int> &a, vector<Ocurrences> &ocurrences,const int &thres) {
  int left=0;
  int right= a.size();
  bool newvalue = true;
  ...
  vector<int>::const_iterator first1 = a.begin();
  vector<int>::const_iterator last1first2 = a.begin() + midpoint;
  vector<int>::const_iterator last2 = a.end();
  vector<int> subvec1(first1,last1first2);
  vector<int> subvec2(last1first2,last2);
  //std::cout << "did this1" << std::endl;
  int subans = get_majority_element(subvec1, ocurrences, thres);
  int subans2 = 0;
  if (subans)
    return 1;
  else
    subans2 = get_majority_element(subvec2, ocurrences, thres);
...
void test_solution(vector<Ocurrences>& ocurrences){
  ...
  get_majority_element(a, ocurrences, n/2);
}
...
int main() {
  ...
  int ans = get_majority_element(a, ocurrences, n/2);
/*  int ans = 0;
  for(int i=0; i<ocurrences.size(); ++i){
