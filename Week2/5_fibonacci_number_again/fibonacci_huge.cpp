...
long long get_fibonacci_mod_fast(long long n, long long m){
    long long newn = n % (long long)periods[m-2];
    
    return get_fibonacci_huge_naive(newn, m);
}

void test_solution(){
   assert(get_fibonacci_mod_fast(11,2) == (long long)1);
   for (int i=0; i<10; i++){
     int n = rand() % 100;
     int m = rand() % 10 + 2;	// 2 is minimum
     assert(get_fibonacci_huge_naive(n,m) == get_fibonacci_mod_fast(n,m));
   }
}
...
int main() {
    long long n, m;
    //test_solution();
    //std::cout << "Passed Test, input numbers." << std::endl;
    std::cin >> n >> m;
    std::cout << get_fibonacci_mod_superfast(n, m) << std::endl;	// no need for the entire periods table.
}
